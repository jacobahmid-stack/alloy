...
On 2026-07-09 AWS shipped **Partner Lead Prospecting** (GA, free to every ACE partner globally: AI sales plays, call scripts, email outreach), on top of **lead enrichment** (2026-06-15: propensity + program/funding/sales-motion eligibility), delivered via the console and a managed AWS Partner Central MCP server. This is the fifth agentic partner release since March. It takes the downstream half of Smith first-party and free. Full analysis: [[aws-pc-commoditization-2026-07]] and `AWS_PDM_CROSSWALK.md` sibling read.
...
**The one thing AWS structurally does NOT do: discovery.** Their entire contract begins at "Submit your leads." Nothing in ten primary sources searches, scans, or finds a net-new company; data is isolated to the partner's own opportunities. So Alloy is not a worse copy of AWS's tool. **Alloy is the layer before it: the machine that manufactures the leads AWS's free machine only enriches.** AWS even hands us the proof metric, its own "leads where enrichment data was not available" miss count.
