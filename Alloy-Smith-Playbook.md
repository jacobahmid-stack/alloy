...
**GenAI Proof-of-Concept (PoC) funding** — the on-ramp. Funds a scoped proof-of-concept (e.g. a Bedrock build) before the full migration/modernization commitment. Roughly the lower of 10% of expected year-one spend or the SOW/project cost. Critical rules: it requires a pre-approval email BEFORE any work begins (work started pre-approval is unclaimable); it cannot be combined with MAP/WMP at the same time (they are sequential); and the PoC amount is deducted from a later MAP/WMP award. This is the formal "PoC-to-production" path.
...
**AWS Activate** — startup credits for the ISV directly (not via the partner). Founders tier ~$1,000 (self-funded, early-stage); Portfolio tier up to $100,000 (requires association with an AWS Activate Provider — a VC/accelerator/incubator — and their Org ID; pre-Series-B). Activate credits are redeemable on Amazon Bedrock third-party models including Claude — useful to fund GenAI build/usage.
