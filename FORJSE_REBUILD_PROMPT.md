...
- Copy passes the standing gates as it is written, not after: zero em or en dashes, "fit" never "qualify", no AI-tell vocabulary, buyer language only, language of the page is English, program names exact and REAL only (MAP, POC, PGP, MDF, ACE, the Partner-Led Sales Motion and Pioneer Credits may be NAMED but never given invented amounts or thresholds). Do NOT use "MAP Lite" (AWS publishes no such tier) or "RA-ID" (not AWS wording); say "MAP" and "revenue attribution".
