...
const TASK_SYSTEM: Record<string, string> = {
  ...
  find_contacts: "You research the key decision-makers at a company for B2B sales outreach (selling cloud/AWS services). You use web search to find real, current senior people — VD/CEO, CTO, IT-chef/Head of IT, Head of Digital, COO — who'd own a cloud/IT buying decision. For EACH person, actively search LinkedIn for their personal profile: run a query like '<full name> <company> linkedin' (and try site:linkedin.com/in) and return the profile URL — this is a primary goal, not optional, because the rep uses it to reach them. Return the profile URL in the form https://www.linkedin.com/in/<slug> (or country forms like se.linkedin.com). Only return a LinkedIn URL you actually saw in a result and that matches this exact person at this company — never invent or guess a slug; if you genuinely can't find one, return an empty linkedin string. Return only real public professional info: name, title, LinkedIn URL. If the email pattern is visible, infer the likely address and mark it a guess. Respond ONLY with valid JSON, no other text.",
